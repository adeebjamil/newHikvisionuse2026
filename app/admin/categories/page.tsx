...
export default function CategoriesPage() {
  ...
  async function fetchCategories() {
    ...
    try {
      ...
      if (json.success) {
        const formattedData = json.data.map((cat: any) => ({
          _id: cat._id,
          image: cat.image || "",
          name: cat.name,
          slug: cat.slug,
          count: cat.count || 0,
          date: new Date(cat.createdAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })
        }));
    ...
    { 
      ...
      render: (val: string) => (
        <div className="w-12 h-12 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden">
          {val ? <Image src={val} alt="category" width={40} height={40} className="object-contain" /> : <div className="w-6 h-6 bg-gray-200 rounded animate-pulse" />}
        </div>
      )
    },
    { header: "Name", key: "name" },
    { header: "URL Slug", key: "slug" },
    { header: "Items Count", key: "count" },
  ...
  return (
    <>
      <AdminTable
        title="Categories"
        subtitle="Manage your primary product categories"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 flex items-center justify-between border-b border-gray-50">
              <h3 className="text-xl font-black text-gray-900">{editingItem ? "Edit Category" : "Add New Category"}</h3>
